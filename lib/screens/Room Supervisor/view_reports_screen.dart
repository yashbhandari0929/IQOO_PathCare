...
class _SupervisorViewReportsScreenState
  ...
  List<Map<String, dynamic>> get _filtered {
    return _reports.where((r) {
      ...
      final patient = ((r['patients'] as Map?)?['full_name'] ?? '')
          .toString()
          .toLowerCase();
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      SizedBox(width: 12),
                      _statPill('Showing', '${filtered.length}'),
                      SizedBox(width: 12),
                      _statPill(
                        'Blood Tests',
                        ...
                        return Padding(
                          ...
                          child: FilterChip(
                            ...
                            onSelected: (_) => setState(
                              () => _filterType = selected ? null : type,
                            ),
                            selectedColor: Colors.teal,
                            backgroundColor: Colors.white,
                            checkmarkColor: Colors.white,
  ...
  Widget _buildCard(Map<String, dynamic> report) {
    ...
    final createdAt =
        DateTime.tryParse(report['created_at'] ?? '') ?? DateTime.now();
    final formatted = DateFormat('dd MMM yyyy').format(createdAt.toLocal());
    final color = _typeColor(report['report_type']);
          ...
          child: Row(
            children: [
              ...
              Column(
                children: [
                  IconButton(
                    icon: Icon(Icons.open_in_new, color: Colors.teal, size: 20),
                    tooltip: 'View',
                    onPressed: () => _openReport(report['file_url'] ?? ''),
                  ),
                  IconButton(
                    icon: Icon(
